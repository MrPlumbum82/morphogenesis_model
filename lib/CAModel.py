...
class CAModel(nn.Module):
    ...
    def perceive_func(self, x, angle):
        ...
        dx = np.outer([1, 2, 1], [-1, 0, 1]) / 8.0  # this is the Sobel filter
        dy = dx.T
    ...
    def update(self, x, fire_rate, angle):
        x = x.transpose(1,3)
        pre_life_mask = self.alive_func(x)

        dx = self.perceive_func(x, angle)
        dx = dx.transpose(1,3)
        dx = self.fc0(dx)
        dx = F.relu(dx)
        dx = self.fc1(dx)
        ...
        post_life_mask = self.alive_func(x)
